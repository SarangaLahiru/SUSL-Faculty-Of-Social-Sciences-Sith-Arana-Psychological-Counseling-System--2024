        ...
        <div>
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="avatarDropdown">
                    <li><a class="dropdown-item" href="{{ route('counsellor.password.request') }}">Reset Password</a>
                    </li>
                    <li>
                        <form action="{{ route('counsellor.logout') }}" method="POST" class="d-inline">
                            @csrf <!-- Include CSRF token for security -->
                            <button type="submit" class="dropdown-item text-danger"
                                    style="border: none; background: none; cursor: pointer;">Logout
                            </button>
                        </form>
                    </li>
                </ul>
        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block sidebar shadow">
            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    <li class="nav-item">
                        <a class="nav-link {{ request()->is('counsellor/dashboard') ? 'active' : '' }}"
                           href="/counsellor/dashboard">
                            Dashboard
                        </a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ request()->is('counsellor/profile') ? 'active' : '' }}"
