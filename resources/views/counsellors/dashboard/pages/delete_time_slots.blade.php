...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('counsellor.deleteTimeSlots') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-danger">Delete All Time Slots</button>
            <a href="{{ route('counsellor.dashboard') }}" class="btn btn-secondary">Cancel</a>
        </form>
    </div>

    <div class="container mt-5">
        <h2>Add New Time Slots</h2>
        <form action="{{ route('counsellor.addTimeSlots') }}" method="POST">
            @csrf
            <div class="mb-3">
